export default function ProjectVisual({ kind, image, large = false }: { kind: 'system' | 'game'; image?: string; large?: boolean }) {
  ...
  if (kind === 'system' && large) return <div className={`game-panel relative grid grid-cols-2 gap-2 overflow-hidden bg-[#182039] p-2 ${container}`}>
  ...
  if (kind === 'system') return <div className={`relative overflow-hidden bg-[#182039] ${container}`}>
    ...
    <span className="hud-label absolute bottom-5 left-6">TOR SYSTEM SCREENSHOT</span>
  </div>
  return <div className={`relative overflow-hidden bg-[#151b33] ${container}`} aria-label="BAYANI placeholder visual" role="img">
    ...
    <span className="absolute bottom-5 left-6 font-mono text-xs font-medium uppercase tracking-[0.2em] text-amber-100/70">GAMEPLAY PLACEHOLDER</span>
  </div>
}
